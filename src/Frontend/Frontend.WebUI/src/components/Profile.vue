<template>
    <div v-show="GET_IS_LOGGED_IN">
        <el-button class="authButton" @click.prevent="logoutWithGoogle">
            Sign out
        </el-button>
        <img class="profilePicture" :src="GET_USER?.profile?.picture" />
    </div>
    <div v-show="!GET_IS_LOGGED_IN">
        <el-button class="authButton" @click.prevent="grantAccess">
            ...
            Grant access by signing in with Google
        </el-button>
    </div>
</template>
...
<script lang="ts">
    ...
    import { toUser, User } from '@/modules/User/User';
    ...
    export default defineComponent({
        name: 'Profile',
        computed: {
            ...mapGetters('user', [UserGetterTypes.GET_IS_LOGGED_IN, UserGetterTypes.GET_USER]),
        },
        methods: {
            ...mapActions('user', [UserActionTypes.SET_USER]),
            async logoutWithGoogle() {
                await this.$gapi.logout();
                this.$store.dispatch(`user/${UserActionTypes.SET_USER}`, null);
            ...
            async grantAccess() {
                ...
                const authorizationDto: QueryResponse<AuthorizationDto> = await this.$authorizationService.AuthorizeCode(code);
                const user: User = toUser(authorizationDto.data);
